...
var styleSwitcher = {
    init: function () {
        ...
        yepnope([
        {
            load: '/static/metis/css/style-switcher.css'
        },
        {
            load: '/static/metis/lib/colorpicker/css/colorpicker.css'
        },
        {
            load: '/static/metis/lib/cssbeautify/cssbeautify.js'
        },
        {
            load: '/static/metis/lib/colorpicker/js/bootstrap-colorpicker.js',
            complete: function () {
                yepnope([
                {
                    load: '/static/metis/less/theme.less'
                },
                {
                    load: '/static/metis/lib/less-1.5.1.min.js',
                    complete: function () {
                        $this.build();
    ...
    build: function () {
        ...
        if (localStor) {
            ...
            if (localStorage.pattern) {
                $('body') .css({
                    'background': 'url(/static/metis/img/pattern/' + localStorage.pattern + '.png) repeat'
                });
            }
        }
        ...
        $.each(patternImages, function (i) {
            var listElement = $('<li/>') .append($('<a/>') .css({
                'background': 'url(/static/metis/img/pattern/' + patternImages[i].image + '.png) repeat'
            }) .attr({
                'href': '#',
        ...
        patternList.find('a') .on('click', function (e) {
            e.preventDefault();
            $('body') .css({
                'background-image': 'url(/static/metis/img/pattern/' + $(this) .data('patternImage') + '.png)',
